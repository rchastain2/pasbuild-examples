...
type TFluidSynth = class
	...
	fluid_player_get_division : function(player : PFluidPlayer) : longint;
...
constructor TFluidSynth.Create;
	...
	procedure _GetFunc(dest : pointer; const funcname : PChar); inline;
	begin
...
// Library names to look out for:
// - Archlinux: libfluidsynth.so -> libfluidsynth.so.3 -> libfluidsynth.so.3.3.2
// - Ubuntu 24.04: libfluidsynth.so.3 -> libfluidsynth.so.3.2.2
// - Fedora 41: libfluidsynth.so.3 -> libfluidsynth.so.3.3.2
// - Windows: libfluidsynth-3.dll
const names : array of PChar = (
	'libfluidsynth.' + SharedSuffix,
	'libfluidsynth-3.' + SharedSuffix,
	'libfluidsynth.' + SharedSuffix + '.3',
	'libfluidsynth-4.' + SharedSuffix, // not defined as of Jan 2025, but will likely be mostly compatible as and when
	'libfluidsynth.' + SharedSuffix + '.4',
	'libfluidsynth-2.' + SharedSuffix,
	'libfluidsynth.' + SharedSuffix + '.2',
	'fluidsynth.' + SharedSuffix
	);
var libname : PChar;
begin
	settings := NIL;
	...
	// Very annoying having to specify these twice! But I haven't found a way to successfully extract a string from the
	// function pointer, or vice versa. Also could not enumerate the method stubs. It's supposed to be possible...
	...
	_GetFunc(@fluid_player_get_division, 'fluid_player_get_division');
